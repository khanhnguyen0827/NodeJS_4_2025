...
const roleService = {
   ...
   findAll: async function async (req) {
 ...
 */
        ...
        Object.entries(filters).forEach(([key, value], index, array) => {
            ...
            if(!isNaN(date(value))){
                console.log('day la ngay');
               const startDate = new Date(value);
               const endDate = new Date(value);
               endDate.setDate(endDate.getDate() + 1);  
               filters[key] = {
                gte: startDate, // Use 'gte' for greater than or equal to
                lte: endDate, // Use 'lte' for less than or equal to
               };
        ...
        const listRole = await prisma.roles.findMany({
